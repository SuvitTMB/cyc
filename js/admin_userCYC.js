...
var db = firebase.firestore().collection("CYCProfile");
var dateString="";
var sCYCStatus = 0;
...
function loadData(x){
  ...
  db.where('CYCStatus','==',parseInt(sCYCStatus))
  .orderBy('EmpID','asc')
  //.limit(20)
  .get().then((snapshot)=> {
    snapshot.forEach(doc=> {
      ...
      if(doc.data().LinePicture !== null) {
        sImg = '<img src="'+ doc.data().LinePicture +'" style="width:35px;height:35px;">';
      }
      var dataSet = [sImg, doc.data().LineName, doc.data().EmpID, doc.data().EmpName, doc.data().EmpGroup, doc.data().DateRegister, "<div class='btn-t1 btn-add' id="+i+">คลิก</div>" , doc.id, i];
...
function ClickID(x,id) {
  ...
  .get().then((snapshot)=> {
    snapshot.forEach(doc=> {
      ...
      str += '<div style="padding:8px; font-weight: 600; color:#0056ff;">'+ doc.data().EmpID +'<br>'+ doc.data().EmpName +'<br>'+ doc.data().EmpGroup  +'</div>';
